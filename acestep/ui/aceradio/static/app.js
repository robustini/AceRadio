...
function renderLoras(items){ const el=qs('loraList'); el.innerHTML=''; items.forEach(item=>{ const id=item.id||item.key||item.name||item.filename; if(!id) return; const label=(item.label||item.name||id); const row=document.createElement('div'); row.className='lora-item'; row.innerHTML=`<label class="lora-main"><input class="lora-check" type="checkbox" data-lora-id="${id}"><span class="lora-name" title="${label}">${label}</span></label><input type="number" min="0" max="2" step="0.05" value="0.60" data-lora-weight="${id}">`; el.appendChild(row); }); }
function collectLoras(){ return [...document.querySelectorAll('[data-lora-id]')].filter(x=>x.checked).map(x=>({id:x.getAttribute('data-lora-id'), weight:Number(document.querySelector(`[data-lora-weight="${x.getAttribute('data-lora-id')}"]`).value||0.6), enabled:true})); }
